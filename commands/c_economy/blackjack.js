  ...
  run: async (bot, message, args, ops) => {
    ...
    try {
      ...
      if (dealerInitialTotal === 21 && playerInitialTotal === 21) {
        ...
        return embed(message, "😂 У вас обоих Блэкджек!", false)
      } else if (dealerInitialTotal === 21) {
        ops.games.delete(message.channel.id);

        return error(message, `У дилера блэкджек!\nВы проиграли **${Math.floor(Math.floor(amount))}**${COIN}`)
      ...
      while (!win) {
        if (playerTurn) {
          ...
          if (hit) {
            ...
            await embed(message, `**Вторая карта дилера ${dealerHand[1].display}, в общем ${dealerTotal}!**`, false);
            playerTurn = false;
          }
        } else {
          ...
          if (total > 21) {
            ...
            await embed(message, `**Дилер достает карты ${card.display}, в общем ${total}!**`, false)
          }
        }
      }
      ...
      if (win === true) {
        ...
        return embed(message, `**${reason}, Вы выиграли ${COIN}${Math.floor(2 * Math.floor(amount))}**!`);
      } else if (!win) {
        return error(message, `**${reason}, Вы проиграли ${COIN}${Math.floor(Math.floor(amount))}**!`);

      } else {
        await mc.giveCoins(message.member.id, message.guild.id, Math.floor(amount));
        return embed(message, `👀 **${reason}, У вас ничья!**`, false);
      }
    } catch (err) {
      ops.games.delete(message.channel.id);
      throw err;
