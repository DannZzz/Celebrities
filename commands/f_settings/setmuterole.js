  ...
  run: async (bot, message, args) => {
    ...
    if (!args[0]) {
      ...
      if (message.guild.roles.cache.has(b)) {
        ...
        return message.channel.send({embeds: [mtsEmbed.setDescription(`${AGREE} Мьют роль для этого сервера: \`${roleName.name}\`!`)]})
      } else {
    ...
    if (!role)
      ...
      try {
        ...
        if (role.id === a) {
          ...
          message.channel.send({embeds: [mtsEmbed.setDescription(
