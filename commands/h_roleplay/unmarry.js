...
module.exports = {
    ...
    run: async (bot, message, args, ops) => {
        ...
        if (!mar2) return error(message, "Вы не женаты.");
      ...
      .setDescription(`Вы уверены, что хотите расстаться? За это вы заплатите 150 ${STAR}`)
      ...
      collector.on("collect", async (i) => {
        if(i.customId === buttonList[0].customId) {
          ...
          if (!curPage.deleted) {
            ...
            curPage.edit({
              embeds: [Emb.setDescription(`Действие успешно отклонено.`)],
          ...
          curPage.edit({
            embeds: [Emb.setDescription(`${message.guild.members.cache.get(user.id)} теперь вы свободны...`)],
            components: [disabledRow],
          });//
          ops.queue.delete(user.id)
          collector.stop()
